...
public class Day :IComparable<Day> {
	...
	public Day(int _number, List<Player> _deathsOvernight, List<Player> _allPlayers, List<Vote> _votes)
	{
		number = _number;
		//voteText = _voteText;
	...
	public static List<Vote> parseVoteText(List<Player> playersAlive, string voteText)
	{
        ...
        do {
			...
			if (Player.FindPlayerByName(players, playerNameVotedFor) == null)
			{
				System.Console.WriteLine("could not find PLAYER:|" + playerNameVotedFor + "|" );
				foreach(Player player in players)
				{
					System.Console.WriteLine("Player in list:|" + player.Name + "|");
				}
			}
			Vote newVote = new Vote(Player.FindPlayerByName(players,playerNameVotedFor),Player.FindPlayerByName(players,playerVoted),postNumber, dateTime,false, debugString);
